...
def load(task_path: str,
  ...
  simulator._adb_controller.execute_command(['root'], with_prefix=False)

  # Load environment.
  return environment.AndroidEnv(coordinator=coordinator)


def execute_command(env: environment.AndroidEnv,
                    args: List[str],
                    timeout: Optional[float] = None,
                    with_prefix=True) -> bytes:
  """Executes an adb command."""

  _adb_controller = env._coordinator._simulator._adb_controller

  timeout = _adb_controller._default_timeout if timeout is None else timeout
  command = self.command_prefix() + args if with_prefix else [self._adb_path] + args
